...
| P1: Platform and scale feasibility | P0 | Throwaway/isolated packaging and benchmark prototypes; decision records | Packaged shell/helpers/media work on minimum OS targets including RHEL route; million-item query and vector candidate measurements; framework/engine/index decisions recorded |
...
incremental discovery at 1M/5M. Compare the current Chroma behavior with embedded
vector candidates under memory, incremental indexing and rare-filter recall tests.
...
Ventura, Windows 10, date-rail coverage, or million-item scale.
...
Exercise a million-row fixture before building all pages. Port shared components
...
codecs/transcode lifecycle and OS-native actions. Test CPU fallback and GPU
...
Run the complete feature matrix, 1M/5M measurements, 10M stress and storage-failure
suite. Report results with hardware and dependency/model revisions. Publish only
