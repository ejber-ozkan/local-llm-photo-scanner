        ...
        <div className="p-8 h-full flex flex-col">
            <header className="mb-6">
                <div className="flex justify-between items-end">
                    <div>
                        ...
                        <p className="text-textMuted">Review identical files and media skipped during library scans.</p>
                    </div>
                </div>
            </header>
            ...
            {loading ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-textMuted bg-surface border border-[#333] rounded-xl p-12">
                    ...
                    <p>Your gallery is clean! We haven't detected any duplicate images or skipped screenshots.</p>
                </div>
            ) : (
                <div className="flex-1 overflow-auto">
                    ...
                    {skipped.length > 0 && (
                        <div className="mb-10">
                            <h2 className="text-xl font-semibold text-red-400 mb-4 flex items-center gap-2">
                                <ShieldAlert className="w-5 h-5" /> Skipped Media ({skipped.length})
                            </h2>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                                {skipped.map((item) => (
                                    <div key={item.id} className="bg-surface rounded-xl overflow-hidden border-2 border-red-500/50 hover:border-red-500 transition-all shadow-lg flex flex-col">
                                        <div className="aspect-square relative bg-black group">
                                            <img
                                                src={`${API_BASE_URL}/api/image/${item.id}?t=${Date.now()}`}
                                                alt={item.filename}
                                        ...
                                        <div className="p-4 bg-[#160000] flex-1 border-t border-red-900/50">
                                            <h3 className="text-white font-medium truncate mb-2">{item.filename}</h3>
                                            <p className="text-xs text-red-300 p-2 bg-red-950/50 rounded border border-red-900/30">
                    ...
                    {duplicates.length > 0 && (
                        <div>
                            <h2 className="text-xl font-semibold text-white mb-4">Duplicate Groups</h2>
                                ...
                                {duplicates.map((group) => (
                                    <div key={group.hash} className="bg-surface rounded-xl overflow-hidden border border-[#333] hover:border-[#444] transition-all shadow-lg flex flex-col">
                                        <div className="aspect-square relative bg-black">
                                            <img
                                                src={`${API_BASE_URL}/api/image/${group.original.id}?t=${Date.now()}`}
