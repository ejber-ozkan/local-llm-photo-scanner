...
import backup_db
import restore_db
from database import DB_FILE, DB_TEST_FILE, find_best_face_match, init_db, init_single_db
from services.image_service import extract_exif_for_filters, extract_gps_from_exif, process_image_with_ollama

# In-memory store for background scanning logs
scan_logs: collections.deque[str] = collections.deque(maxlen=500)
...
async def get_photo_detail(photo_id: int):
    ...
    cursor.execute(
        "SELECT id, filepath, filename, description, gps_lat, gps_lon, ai_model FROM photos WHERE id = ?", (photo_id,)
    )
                        ...
                        for tag_id, value in ifd.items():
                            ...
                            if tag == "ExposureTime":
                                ...
                                with contextlib.suppress(Exception):
                                    metadata["F-stop"] = f"f/{float(value):.1f}"
                            elif tag == "ISOSpeedRatings":
                                metadata["ISO Speed"] = f"ISO-{value}"
                            elif tag == "FocalLength":
                                with contextlib.suppress(Exception):
                                    metadata["Focal Length"] = f"{float(value):.1f} mm"
...
async def get_scan_status():
    ...
    return {"state": SCAN_STATE, "total": total, "processed": processed, "pending": pending}
...
async def get_backups():
    ...
    for f in os.listdir("backups"):
        if f.endswith(".db"):
            ...
            files.append({"filename": f, "size_bytes": size, "created_at": datetime.fromtimestamp(created).isoformat()})
...
async def scan_single_photo(file: UploadFile = File(...), model: str | None = Form(None)):
    ...
    try:
        cursor.execute(
            "INSERT INTO photos (filepath, filename, status) VALUES (?, ?, 'pending')", (file_path, isolated_filename)
        )
                    ...
                    try:
                        ...
                        for tag_id, value in ifd.items():
                            ...
                            if tag == "ExposureTime":
                                try:
                                    v = float(value)
                                    metadata_extracted["Exposure Time"] = (
                                        f"1/{int(1 / v)} sec." if v > 0 and v < 1 else f"{v} sec."
                                    )
                                except Exception:
                                    pass
                            elif tag == "FNumber":
                                with contextlib.suppress(Exception):
                                    metadata_extracted["F-stop"] = f"f/{float(value):.1f}"
                            elif tag == "ISOSpeedRatings":
                                metadata_extracted["ISO Speed"] = f"ISO-{value}"
                            elif tag == "FocalLength":
                                with contextlib.suppress(Exception):
                                    metadata_extracted["Focal Length"] = f"{float(value):.1f} mm"
                            elif tag == "DateTimeOriginal":
    ...
    try:
        ...
        cursor.execute(
            "SELECT id, description, date_created, date_modified FROM photos WHERE file_hash = ? AND status = 'processed' AND ai_model = ? LIMIT 1",
            (file_hash, model_to_use),
        )
        existing_photo = cursor.fetchone()
        ...
        if existing_photo:
            ...
            cursor.execute(
                "SELECT entity_type, entity_name, bounding_box FROM entities WHERE photo_id = ?", (existing_id,)
            )
            ...
            history = []
            cursor.execute(
                "SELECT id, description, ai_model FROM photos WHERE file_hash = ? AND status = 'processed' AND id != ?",
                (file_hash, existing_id),
            )
            historical_scans = cursor.fetchall()
            for hs_id, hs_desc, hs_model in historical_scans:
                cursor.execute(
                    "SELECT entity_type, entity_name, bounding_box FROM entities WHERE photo_id = ?", (hs_id,)
                )
                ...
                history.append(
                    {
                        ...
                        "description": hs_desc,
                        "entities": [{"type": t, "name": n, "bounding_box": b} for t, n, b in h_ent],
                    }
                )
    ...
    ai_response = process_image_with_ollama(file_path, OLLAMA_URL, model_to_use)
    ...
    try:
        if "Entities:" in description:
            ...
            if not any(entities_part.lower().startswith(p) for p in ["no ", "none", "n/a"]) and (
                "no pets" not in entities_part.lower()
                and "no dogs or cats" not in entities_part.lower()
                and "no pets," not in entities_part.lower()
            ):
                    ...
                    entities_found.append(
                        {
                            "type": "person",
                            "name": matched_name,
                            "bounding_box": json.dumps(facial_area) if facial_area else None,
                        }
                    )
        except ValueError:
            pass  # No face found
