...
def seed_extra_db(db_path, dummy_img_path):
    ...
    c = conn.cursor()
    c.execute("INSERT INTO photos (id, filepath, filename, description, status, date_created, date_modified, date_taken) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
              (10, dummy_img_path, "dummy.jpg", "A dummy photo", "processed", "2025-01-01", "2025-01-01", "2025-01-01 12:00:00"))
    
    c.execute("INSERT INTO entities (photo_id, entity_type, entity_name, first_name, last_name, bounding_box) VALUES (?, ?, ?, ?, ?, ?)",
...
def test_get_image_from_test_db(client, mock_db_file, dummy_img):
    ...
    test_db = photo_backend.DB_TEST_FILE
    conn = sqlite3.connect(test_db)
    cursor = conn.cursor()
    cursor.execute("DELETE FROM photos WHERE id = 555")
    cursor.execute("INSERT INTO photos (id, filepath, filename, status) VALUES (555, ?, 'testonly.jpg', 'processed')", (unique_dummy,))
    conn.commit()
    conn.close()
    ...
    if resp.status_code != 200:
        json_resp = resp.json()
        conn = sqlite3.connect(photo_backend.DB_TEST_FILE)
        row = conn.cursor().execute("SELECT id, filepath FROM photos WHERE id=555").fetchone()
        conn.close()
        import os
        assert resp.status_code == 200, f"Failed. JSON={json_resp}, DB_TEST_FILE={photo_backend.DB_TEST_FILE}, row={row}, exists={os.path.exists(row[1] if row else '')}"
...
def test_scan_status_and_logs(client):
    ...
    assert resp.status_code == 200
    assert type(resp.json()["logs"]) == list
...
def test_database_backups_and_restore(client, monkeypatch):
    ...
    def fake_exists(*args, **kwargs): return True
    def fake_listdir(*args, **kwargs): return ["backup1.db", "backup2.db"]
    def fake_getsize(*args, **kwargs): return 1000
    def fake_getctime(*args, **kwargs): return 1600000000
    ...
    def fake_restore(*args, **kwargs): return True
...
def test_get_duplicates(client, mock_db_file, dummy_img):
    ...
    c = conn.cursor()
    c.execute("INSERT INTO photos (id, filepath, filename, file_hash, file_size, status) VALUES (?, ?, ?, ?, ?, ?)",
