import pytest
from photo_backend import background_processor, init_db
import sqlite3
import os
import shutil
...
def seed_db_for_processing(db_file, filepath):
    ...
    c = conn.cursor()
    c.execute("INSERT INTO photos (filepath, filename, status) VALUES (?, ?, 'pending')", 
...
def test_background_processor_success(mock_db_file, test_image, mock_ollama, monkeypatch):
    ...
    def fake_find(img_path, db_path, model_name="VGG-Face", distance_metric="cosine", enforce_detection=False, detector_backend="ssd", align=True, silent=False):
        return []
    ...
    monkeypatch.setattr("photo_backend.ACTIVE_OLLAMA_MODEL", "mock_model")
    monkeypatch.setattr("photo_backend.SCAN_STATE", "running")
